...
module.exports = class extends Command {
  ...
  async run(msg, lang, args, sendDeletable) {
    ...
    const embed = new Discord.RichEmbed()
      .setTitle(f(lang['battlepass'], config.battlepass.currentSeason) + `${msg.author.premium ? ' (Premium)' : ' (FreePass, buy Discord Nitro for upgrade!... not yet.)'} - ${lang['tier']} ${user.bp_tier || Math.floor(Math.sqrt(4 + user.exp/2)-1)}`)
