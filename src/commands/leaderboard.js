...
module.exports = class extends Command {
  ...
  async run(msg, settings, user, lang, args) {
    if (args[1] === 'server') {
      ...
      if (s_points[4]) embed.addField(':first_place:', `${s_points[4]} points (${getServer(s_ids[4])})`)
      if (s_points[3]) embed.addField(':second_place:', `${s_points[3]} points (${getServer(s_ids[3])})`)
      if (s_points[2]) embed.addField(':third_place:', `${s_points[2]} points (${getServer(s_ids[2])})`)
      if (s_points[1]) embed.addField('<:fourth_place:534409887027953694>', `${s_points[1]} points (${getServer(s_ids[1])})`)
      if (s_points[0]) embed.addField('<:fifth_place:534410165169029120>', `${s_points[0]} points (${getServer(s_ids[0])})`)
      ...
      if (u_points[4]) embed.addField(':first_place:', `${u_points[4]} points (${getUser(u_ids[4])})`)
      if (u_points[3]) embed.addField(':second_place:', `${u_points[3]} points (${getUser(u_ids[3])})`)
      if (u_points[2]) embed.addField(':third_place:', `${u_points[2]} points (${getUser(u_ids[2])})`)
      if (u_points[1]) embed.addField('<:fourth_place:534409887027953694>', `${u_points[1]} points (${getUser(u_ids[1])})`)
      if (u_points[0]) embed.addField('<:fifth_place:534410165169029120>', `${u_points[0]} points (${getUser(u_ids[0])})`)
      msg.channel.send(embed)
    }
  }
}
